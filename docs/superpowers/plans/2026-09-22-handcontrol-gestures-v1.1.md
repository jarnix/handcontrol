...
- Same as the v1.0 plan: pure modules never import cv2/mediapipe/pystray/ctypes; tests need no camera; every threshold comes from `Settings` with the defaults of spec §6; `uv run pytest -q` green at the end of every task; commit per task with the `Co-authored-by: Claude Fable 5.1 <noreply@anthropic.com>` trailer (Julien's choice, 2026-09-23); create files with the Write tool (the Bash tool breaks on apostrophes).
